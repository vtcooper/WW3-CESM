...
!          VTC change to only have viscous damping from T2 T4    *******************
...
!             KARG1(ik)=min(karg1(ik),0.0)
              ALPHA(ik) = 0.0 !10.0**KARG1(ik)               ! VTC *****************
!             WN_I(ik)  = 10.0**KARG1(ik)  ! dont use this, correction ALPHA vs WN_I
!	      if (WN_I(ik).gt.0.9) then
!	        write(*,*) 'whacky',WN_I(ik),x1,x2,x3
!              endif 
	      perfour=x1sqr*x1sqr
              if (x1.gt.0.0) ALPHA(ik) = amhb/x1sqr + bmhb/perfour !! VTC only T2T4*
